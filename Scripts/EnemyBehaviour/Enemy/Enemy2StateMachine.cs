...
[RequireComponent(typeof(EnemyThrowShockwaveState))]
[RequireComponent(typeof(EnemyHurtState))]
[RequireComponent(typeof(EnemyDieState))]
public class Enemy2StateMachine : AbstractStateMachine
{
	#region Exposed
	#endregion
	...
	protected override void Awake()
	{
		...
		_enemyShockwave = GetComponent<EnemyThrowShockwaveState>();
	...
	protected override void OnStateUpdate()
	{
		switch (_currentState)
		{
			case State.LOCOMOTION:
				...
				if (_enemyShockwave.CanThrowShockwave() && _enemyController.GetTargetDistance() >= _enemyController.GetDetectionDistance() / 4
														&& _enemyController.GetTargetDistance() < _enemyController.GetDetectionDistance()*2 / 4)
				{
					TransitionToState(State.THROW);
				}
			...
			case State.THROW:
				//_enemyShockwave.DoUpdate();
				if (_enemyShockwave.CanMove())
				{
					TransitionToState(State.LOCOMOTION);
				}
				break;
	...
	private EnemyThrowShockwaveState _enemyShockwave;
